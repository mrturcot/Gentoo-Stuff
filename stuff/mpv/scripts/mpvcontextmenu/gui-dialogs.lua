...
local function createDialog(selType, selMode)
    ...
    dialogResponse.stdout = string.gsub(dialogResponse.stdout, "[\r\n]", "|")
	-- Remove any trailing newlines, as can happen with Zenity output under Windows.
	while (string.match(dialogResponse.stdout, "|$")) do
		dialogResponse.stdout = string.gsub(dialogResponse.stdout, "|$", "")
	end
    ...
    if (selMode == "add") then
        if ((selType == "file" ) or (selType == "folder")) then
            for filename in string.gmatch(dialogResponse.stdout, "[^|]+") do
...
mp.add_key_binding(opt.addFiles, "add_files_dialog", function() createDialog("file", "add") end)
